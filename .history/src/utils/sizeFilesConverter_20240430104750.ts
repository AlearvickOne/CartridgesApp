const sizeFilesConverter = (file: File) => {
  ...
  if (file !== undefined) {
    ...
    if (bytesGetFile >= kb && bytesGetFile < converter.mb) size = roundedSize(bytesGetFile, kb);
    else if (bytesGetFile >= converter.mb && bytesGetFile < converter.gb)
      size = roundedSize(bytesGetFile, converter.mb);
    else size = roundedSize(bytesGetFile, converter.gb);
  }
};
