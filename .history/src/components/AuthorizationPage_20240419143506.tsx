  ...
  return (
    <main className={styles.authContainer}>
      <div className={styles.authForm_wrapper}>
        ...
        <p className="mb-5 text-center text-white text-xl">{isError ? "Не верные данные" : ""}</p>
        <form
          action={async (formData) => {
            const res = await handleSubmit(formData.get("login"), formData.get("password"));
            setIsError(typeof res !== "undefined" ? true : false);
          }}
          className={styles.authForm}
        >
          <input type="text" name="login" placeholder="Введите логин" />
          <input type="text" name="password" placeholder="Введите пароль" />
          <button type="submit" onClick={() => setIsError(false)}>
            {isError ? <SyncIcon className="animate-spin" /> : "Войти в систему"}
          </button>
        </form>
      </div>
    </main>
