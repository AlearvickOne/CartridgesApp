...
export const OrdersBasketWindow = ({ isActiveWindowProducts, isAnim }: IOrdersBasketWindow) => {
  ...
  const [valute, setValute] = useState({ rub: 0, usd: 0 });

  useEffect(() => {
    setValute({
      rub: reduceSumValue(ordersInBasket, "rub"),
      usd: reduceSumValue(ordersInBasket, "usd"),
    });
  }, [reduceSumValue(ordersInBasket, "rub")]);
      ...
      {isActiveWindowProducts && (
        ...
        >
          ...
          <p className="my-2 text-white flex items-center justify-center">
            Общая сумма {valute.rub}
            <RubleIcon sx={{ fontSize: "14px", margin: "0 2px", color: "white" }} /> - {valute.usd}{" "}
            USD
          </p>
