...
export const ButtonShop = () => {
  const [quantityShop, setQuantityShop] = useState<number>(14);

  const quantityStyle = (quantity: number): string => {
    if (quantity < 10) return "min-w-1 min-h-1";
    else if (quantity >= 10 && quantity < 100) return "min-w-[3rem] min-h-2";
  ...
  return (
    <div className="fixed bottom-10 right-20 z-50">
      <button className="relative border-2 border-black p-4 rounded-[12] ">
        <p
          className={`absolute bottom-[3rem] right-[3.2rem] rounded-full bg-red-600 p-1 ${quantityStyle(
            quantityShop
          )}`}
        >
          {quantityShop}
