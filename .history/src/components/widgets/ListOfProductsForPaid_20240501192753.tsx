...
import stylesScrollBar from "@/styles/styles-scrollbar.module.scss";
  ...
  return (
    <>
      {isActiveWindowProducts && (
        <div
          className={`fixed bottom-32 right-20 z-50 p-5 border-2 rounded-xl ${stylesGradien.gradientToBottom} ${stylesAnims.scaleUpBottomRight} flex justify-center flex-col`}
        >
          <ul
            className={`border-2 py-2 px-3 w-80 h-80  mb-2 rounded-lg bg-white ${stylesScrollBar.scrollbarThin} overflow-auto`}
          >
            {arratProducts.map(({ title, price }) => (
              <li key={} className="border-2 p-2 flex justify-between mb-2 ">
                <h6>title</h6>
                <p>2222 R</p>
                <button>
                  <DeleteIcon />
      ...
      <div className="fixed bottom-10 right-20 z-50">
        <button
          ...
          onClick={() => setIsActiveWindowProducts(!isActiveWindowProducts)}
        >
          <p
