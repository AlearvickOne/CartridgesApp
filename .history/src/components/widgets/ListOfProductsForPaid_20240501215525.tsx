...
export const ListOfProductsForPaid = () => {
  ...
  const handleToggle = () => {
    setIsAnimEnd(true); // Начинаем анимацию
    setTimeout(() => {
      setIsActiveWindowProducts(!isActiveWindowProducts); // Изменяем состояние после завершения анимации
      setIsAnimEnd(false); // Завершаем анимацию
      ...
      {isActiveWindowProducts && (
        <div
          className={`fixed bottom-32 right-20 z-50 p-5 border-2 rounded-xl ${
            ...
            isAnimEnd && stylesAnims.scaleDownBottomRight
