...
export const ButtonShop = () => {
  ...
  const quantityStyle = (quantity: number | undefined): string => {
    ...
    if (quantity < 10) return red + " min-w-[2.2rem] min-h-2";
    else if (quantity >= 10 && quantity < 100) return red + " min-w-[2.3rem] min-h-2";
    else return red + " min-w-2 min-h-2";
  };
  ...
  return (
    <div className="fixed bottom-10 right-20 z-50">
      <button className="relative border-2 border-black p-4 rounded-[12] ">
        <p
          className={`absolute bottom-[2.4rem] right-[2.2rem] rounded-full border-[2px] p-1 ${quantityStyle(
            quantityShop
          )} text-white`}
        >
          {quantityShop}
