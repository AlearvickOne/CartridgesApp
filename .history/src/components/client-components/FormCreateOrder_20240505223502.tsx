...
export const FormCreateOrder = ({ createOrderForm }: IFormCreateOrder) => {
  ...
  const [errorInput, setErrorInputs] = useState({ errorTitle: false, errorPrice: false });
  ...
  const titleChange = (e: TReactEvent) => {
    if (inputsValue.titleValue.length < 3)
      setErrorInputs((prev) => ({ ...prev, errorTitle: true }));
    else setErrorInputs((prev) => ({ ...prev, errorTitle: false }));
  ...
  const priceChange = (e: TReactEvent) => {
    if (inputsValue.priceValue.toString().length < 3)
      setErrorInputs((prev) => ({ ...prev, errorPrice: false }));
    else setErrorInputs((prev) => ({ ...prev, errorPrice: false }));
      ...
      >
        ...
        <div className="flex justify-between items-center">
          ...
          <button
            ...
            disabled={errorInput.errorPrice || errorInput.errorTitle}
