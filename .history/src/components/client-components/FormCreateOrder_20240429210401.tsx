...
import React, { LegacyRef, RefAttributes, useCallback, useRef, useState } from "react";
import { YandexMaps } from "../widgets/YandexMaps";

export const FormCreateOrder = ({ createOrderfn }: IFormCreateOrder) => {
  const [address, setAddress] = useState<string>("");
  const refInputFile = useRef<RefAttributes<HTMLInputElement>>();
  ...
  const handleSetImageFileRef = () => {
    const current = refInputFile.current as HTMLInputElement["files"];
    if (current !== null) {
      const file = current !== null ? current.files[0] : null;
    }
