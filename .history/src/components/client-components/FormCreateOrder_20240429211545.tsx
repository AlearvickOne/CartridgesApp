...
import React, { RefAttributes, useCallback, useRef, useState } from "react";
import { YandexMaps } from "../widgets/YandexMaps";

interface IImageFileRef extends RefAttributes<HTMLInputElement>, HTMLInputElement {}

export const FormCreateOrder = ({ createOrderfn }: IFormCreateOrder) => {
  const [address, setAddress] = useState<string>("");
  const refInputFile = useRef<IImageFileRef>();
  ...
  const handleSetImageFileRef = () => {
    const file = refInputFile.current.(refInputFile.current?.files !== null && files[0]);
