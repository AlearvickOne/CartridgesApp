...
export const FormCreateOrder = ({ createOrderfn }: IFormCreateOrder) => {
  ...
  const refInputFile = useRef<MutableRefObject<LegacyRef<HTMLInputElement>> | null>(null);
  ...
  const handleSetImageFileRef = () => {
    // const currentFiles = (refInputFile.current as unknown) as HTMLInputElement;
    // if (currentFiles.files === null) return;
    // const file = currentFiles?.files[0];
