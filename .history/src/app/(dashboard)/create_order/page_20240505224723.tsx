...
import { ErrorStoreClass } from "@/stores/errorStore";
import { ZodError } from "zod";

export default async function CreateOrder() {
  let errorValid = "no error";
  ...
  async function createOrderForm(formdata: FormData) {
    ...
    try {
      ...
      if (error instanceof ZodError) {
        ErrorStoreClass.setError(
          "Проверьте введенные данные, возможно вы ввели недопустимые значения"
        );
        throw new Error(ErrorStoreClass.error);
      }
    }
  }
  ...
  return (
    <div className=" overflow-auto max-h-screen p-10">
      <FormCreateOrder createOrderForm={createOrderForm} errorValid={errorValid} />
    </div>
  );
}
